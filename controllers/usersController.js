...
export const createUser = asyncHandler(async (req, res, next) => {
    ...
    res.status(200).json({
        success: true,
        data: `${user._id} was added successfully`,
    });
});
...
export const getUser = asyncHandler(async (req, res, next) => {
    ...
    if (!user) {
        return next(
            new Error(
                `User that end with '${req.params.id.slice(-6)}' not found`
            )
        );
    }
...
export const getUserByQuery = asyncHandler(async (req, res, next) => {
    ...
    if (!query.hasOwnProperty("email") && !query.hasOwnProperty("passportID")) {
        return next(
            new ErrorResponse("Params can only be Email/passportID"),
            401
        );
    }
    const user = await User.find(query).populate({
        path: "accounts",
    });
    if (!user) {
        return next(
            new ErrorResponse(`User with this params was not found`, 404)
        );
    }
...
export const deleteUser = asyncHandler(async (req, res, next) => {
    ...
    if (!user) {
        return next(
            new ErrorResponse(
                `User that ends with '${req.params.id.slice(
                    -6
                )}' was not found`,
                404
            )
    ...
    if (!accounts.acknowledged) {
        return next(
            new ErrorResponse(
                `Error occurred while deleting user accounts`,
                400
            )
        );
    }
